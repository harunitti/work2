...
- マップ作成ツール(tool.html)で地点情報のCSVデータを作成します。
- 地点情報にはテキスト、写真、アイコンを紐付けます。
- CSVデータ1ファイルで1カテゴリとなります
- 集めたCSVデータを1つのJsonファイルに書き出します。
- マップ画面(index.html)でjsonファイルを読み込んでオリジナルのGoogleマップを表示します。
- メニューからカテゴリを切り替えることができます。
- モバイル端末でアクセスした場合は、現在地を青いアイコンで表示し、追跡します。
...
- 地図上でダブルクリックした位置に新規マーカーを作成します。
- マーカーをドラッグ&ドロップしてマーカーの位置を調整してください。
- マーカー編集・削除は、対象マーカーを長押しして表示されるモダールウインドウのフォームから可能です。
- 「情報ウインドウ表示」ボタン を押すと全てのマーカーの情報ウインドウを表示・非表示します。
